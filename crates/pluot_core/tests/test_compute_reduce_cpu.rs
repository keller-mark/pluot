...
async fn test_reduce_max_empty() {
    assert_eq!(reduce_max(None, Arc::new(vec![])).await, f32::NEG_INFINITY);
}
...
async fn test_reduce_sum_single() {
    let input = Arc::new(vec![5.5]);
    assert_eq!(reduce_sum(None, input).await, 5.5);
}

#[tokio::test]
async fn test_reduce_sum_empty() {
    assert_eq!(reduce_sum(None, Arc::new(vec![])).await, 0.0);
}
...
async fn test_reduce_extent_empty() {
    assert_eq!(
        reduce_extent(None, Arc::new(vec![])).await,
...
async fn test_histogram_known_extent_single_bin() {
    let input = Arc::new(vec![1.0, 2.0, 3.0]);
...
async fn test_histogram_known_extent_empty() {
    let bins = reduce_histogram_with_known_extent(None, Arc::new(vec![]), 4, 0.0, 10.0).await;
