...
pub fn get_or_init_store(name: &str, wait_for_store_gets: bool, wait_for_store_pushes: bool) -> Arc<AsyncZarritaStore> {
    ...
    if let Some(store) = map.get(name) {
        ...
        map.entry(name.to_string())
            .or_insert_with(|| Arc::new(AsyncZarritaStore::new(name.to_string(), wait_for_store_gets, wait_for_store_pushes)))
            .clone()
    }
}
