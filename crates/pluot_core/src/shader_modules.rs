...
pub mod bitmask_channel {
    ...
    /// `get_channel_{{stroke_or_fill_property}}_{{c_idx}}` (one per channel,
    /// generated by [`crate::emphasis_mode::prepare_emphasis_criteria`] with a
    /// channel-specific `fn_name`) matching `channel_index`. Used for
    /// `is_filtered_in`/`is_selected_in` (see
    /// `.claude/skills/pluot-filter-select-highlight`). Template: substitute
    /// `{{stroke_or_fill_property}}` with `is_filtered_in` or `is_selected_in`
    /// and `{{switch_cases}}` as above.
