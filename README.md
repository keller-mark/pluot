...
- __Layer-based API__: Compose the built-in layers to create complex plots, or build your own layers with full control over the WebGPU shaders, buffers, render pipeline, and draw calls. (Usage of WebGPU compute (GPGPU) operations prior to each layer's draw call is also supported.)
