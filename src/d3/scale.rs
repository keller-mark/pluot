...
pub fn tick_format(start: f64, stop: f64, count: usize, specifier: Option<&str>) -> impl Fn(f64) -> String {
    let step = tick_step(start, stop, count);
    ...
    let precision = if let Some(idx) = specifier.find('.') {
        if let Some(end) = specifier[idx + 1..].chars().find(|c| !c.is_digit(10)) {
            specifier[idx + 1..specifier.len() - end.len_utf8()].parse::<usize>().unwrap_or(0)
        } else {
