...
- `render`: Or its bound variant (e.g., `render_py`, `render_wasm`, etc.) which handles serialization/deserialization or other platform-specific things.

- `pick`: Perform "picking" to look-up the value at a specific (relative) screen coordinate. Used during hover and click event handling (e.g., to show tooltips).
